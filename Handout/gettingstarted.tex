...
\newcommand{\WindowsKnimeInstallerName}{Windows / KNIME\ 3.5.3\ Installer\ (64bit).exe}	
\newcommand{\WindowsOpenMSInstallerName}{Windows / OpenMS-2.3.0-Win64.exe}	
...
\newcommand{\MacKnimeInstallerName}{Mac / knime\_3.5.3.app.macosx.cocoa.x86\_64.dmg}	
\newcommand{\MacOpenMSInstallerName}{Mac / OpenMS-2.3.0-Darwin.dmg}
\newcommand{\KnimeUpdateSite}{http://update.knime.com/analytics-platform/3.5}
\newcommand{\KnimeTrustedSite}{http://update.knime.com/community-contributions/trusted/3.5}
\newcommand{\KnimeTrunkSite}{https://abibuilder.informatik.uni-tuebingen.de/archive/openms/knime-plugin/updateSite/experimental/feature/msstats/}

% enable this line if the internal OpenMS update site is used
%\newcommand{\OpenMSSite}{https://abibuilder.informatik.uni-tuebingen.de/archive/openms/knime-plugin/updateSite/RC/}
...
If the app still does not open, you might need to move them from \directory{Applications / OpenMS-2.3.0} to e.g. your Desktop and back.}
...
\note{If you have installed OpenMS on Linux or macOS via your package manager (for instance by installing the \texttt{OpenMS-2.3.0-Linux.deb} package), then you need to set the \texttt{OPENMS\_DATA\_PATH} variable to the directory containing the shared data (normally \texttt{/usr/share/OpenMS}). This must be done prior to running any TOPP tool.}
...
\begin{itemize}
\item Start \OPENMSTOOL{TOPPView} (see \textbf{Windows}' Start-Menu or \directory{Applications/OpenMS-2.3.0} on \textbf{macOS})
...
Afterwards open your R installation. If you use Windows, you will find an "R x64 3.4.3" icon on your desktop. If you use macOS, you will find R in your Applications folder. In R type the following lines (you might also copy them from the file \directory{R / install\_R\_packages.R} folder on the USB stick):
...
In KNIME, click on \menu{KNIME > Preferences}, select the category \menu{KNIME > R} and set the "Path to R Home" to your installation path. You can use the following settings, if you installed R as described above:
\begin{itemize}
